...
select brand_tag, count(product_tag)  as 'gfgdata' 
from gfgdata
group by brand_tag
order by gfgdata desc limit 5;

-- 5. Finding the top 5 brand who sold the most number of products
select brand_tag, sum(rating_count)  as 'products_sold' from gfgdata 
group by brand_tag
order by products_sold desc limit 5;

-- 6. Finding the top 5 most expensive brands based on their discounted price
select brand_tag, round(avg(discounted_price))  as 'average_price'  from gfgdata 
group by brand_tag
...
select product_tag, sum(rating_count)  as 'product_sold' from gfgdata
group by product_tag
order by product_sold desc limit 5;

-- 9. Finding the top 10 brands who gives maximum discount
select brand_tag, avg(discount_percent)  as 'avg_discount' from gfgdata 
group by brand_tag
order by avg_discount desc limit 5;

-- 10. Finding the top 5 most expensive product categories
select product_tag, avg(discounted_price)  as 'avg_price' from gfgdata 
group by product_tag
...
select brand_tag, product_tag, sum(rating_count) as 'people_rated' from gfgdata 
...
select product_name ,count(product_name) as 'name_count' from gfgdata 
group by product_name 
order by name_count desc limit 10;

-- 18. Number of products sold for every rating (0 - 5)
select rating ,count(rating) as 'freq' from gfgdata
group by rating 
order by rating asc;
...
select rating ,count(rating_count) as 'freq' from gfgdata 
where  product_tag = 'tshirts'
group by rating 
order by rating asc;
